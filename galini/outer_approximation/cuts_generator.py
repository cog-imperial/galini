...
class OuterApproximationCutsGenerator(CutsGenerator):
    ...
    def generate(self, run_id, problem, relaxed_problem, mip_solution, tree, node):
        for i, v in enumerate(relaxed_problem.variables):
            vv = relaxed_problem.variable_view(v)
            if vv.domain != Domain.REAL:
                vv.fix(mip_solution.variables[i].value)

        f_x_solution = self._nlp_solver.solve(relaxed_problem)
        for v in relaxed_problem.variables:
            relaxed_problem.unfix(v)
        ...
        fg = relaxed_problem.expression_tree_data().eval(x_k, f_idx + g_idx)
    ...
    def _generate_cut(self, i, constraint, x, x_k, g, g_x, w, is_objective):
        w[i] = 1.0
        d_g = g.reverse(1, w)
        w[i] = 0.0
        cut_name = '_outer_approximation_{}_round_{}'.format(i, self._round)
        if not is_objective:
            return Cut(
                CutType.LOCAL,
                cut_name,
                LinearExpression(x, d_g, -np.dot(d_g, x_k) + g_x[i]),
